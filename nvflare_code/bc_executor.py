...
class GMICFederatedExecutor(Executor):
    ...
    def __init__(
            ...
            results_dir: str = "/workspace/gmic_results",
            ...
            self.results_dir = chosen_results or "/workspace/gmic_results"
            self.epochs = epochs
    ...
    def initialize(self):
        ...
        try:
            ...
            print(f"[EXEC][WARN] Failed to recover persisted best metrics: {e}")
        ...
        if (not self.load_checkpoint) and getattr(self, 'pretrained_model_index', None):
            try:
                ...
                print(f"[EXEC] Loaded pretrained model index {self.pretrained_model_index}")
            except Exception as e:
                print(f"[EXEC][WARN] pretrained load failed: {e}")
        if self.load_checkpoint and os.path.isfile(self.load_checkpoint):
            try:
                ...
                print(f"[EXEC] Loaded checkpoint: {self.load_checkpoint}")
            except Exception as e:
                print(f"[EXEC][WARN] Failed to load checkpoint {self.load_checkpoint}: {e}")
        ...
        # 7. Loss & optimizer
# 7. Loss & optimizer (FIXED: Default to CrossEntropy for GMIC)
        ...
        if loss_name in ["cross_entropy", "ce"]:
            ...
            print(f"[EXEC][WARN] Unknown loss '{loss_name}', defaulting to CrossEntropyLoss")
            self.criterion = nn.CrossEntropyLoss()
        class _OptArgs: pass
        oa = _OptArgs()
        ...
        if self.tb_log_dir and not self.disable_tensorboard:
            try:
                ...
                if self.tb_writer:
                    print(f"[EXEC][tensorboard] logging to {self.tb_log_dir}")
            except Exception as e:
                print(f"[EXEC][tensorboard][WARN] {e}")
        ...
        if self.gpus:
            try:
                ...
                print(f"[EXEC][WARN] Invalid gpus string: {self.gpus}")
        if torch.cuda.is_available() and len(gpu_ids) > 0:
            # Ensure primary device matches first GPU id
            self.device = torch.device(f"cuda:{gpu_ids[0]}")
            self.model.to(self.device)
            if len(gpu_ids) > 1:
                print(f"[EXEC] Enabling DataParallel on GPUs: {gpu_ids}")
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        try:
            ...
            # Backward compatibility: also include plain val_auc (already covered), but ensure key exists
            ...
            # Use underlying to avoid 'module.' prefixes in keys.
            ...
            # Guarantee presence of 'val_auc' (preferred by selector) if we only have val_auc from prefix
            ...
            try:
                # Build a concise preview (limit number of metrics to avoid log spam)
                preview_items = []
                for i, k in enumerate(sorted(numeric_metrics.keys())):
                    if i >= 25:  # cap preview length
    ...
    def _local_train(self, fl_ctx, abort_signal, shareable: Shareable):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, (inputs, targets, metadata) in enumerate(self.data_loader.get_batch_iterator('train')):
                ...
                if self.use_amp and self._scaler is not None:
                    ...
                    if (batch_idx + 1) % accumulate == 0:
                        self.optimizer.step()
                effective_loss = loss.item() * accumulate  # scale back for logging/aggregation
                bsz = inputs.size(0)
                total_samples += bsz
        ...
        best_val = self.early_stopper.best if (has_val and getattr(self.early_stopper, 'best_state', None)) else (round_best_val if round_best_val > -float('inf') else 0.0)
    ...
    def _save_best_model(self, fl_ctx, metrics, shareable: Shareable, model_type="best"):
        ...
        # Enrich metrics with explicit val_/test_ keys for easier later recovery
        enriched = {
            "client": client_name,
            "round": current_round,
    ...
    def _recover_persisted_best_metrics(self):
        ...
        if not os.path.isdir(persistent_dir):
            return
        # Expected filenames
        # <client>_best_val_overall_gmic_metrics.json
        # <client>_best_test_overall_gmic_metrics.json
        # We don't have fl_ctx here, so simple prints (initialize context) are fine.
        try:
            # Attempt to infer client name from any existing best file if identity not yet available
            # or fallback to scanning directory.
            best_val_file = None
            best_test_file = None
            ...
            if best_test_file and os.path.isfile(best_test_file):
                ...
                if isinstance(auc_val, (int, float)) and auc_val > self.best_metrics.get('test_auc', 0.0):
                    ...
                    updated = True
            if updated:
                print(f"[EXEC] Recovered persisted best metrics: {self.best_metrics}")
        except Exception as e:
            print(f"[EXEC][WARN] Could not recover best metrics: {e}")