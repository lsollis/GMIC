...
def crop_mammogram_one_image_short_path_tolerant(scan, input_data_folder, output_data_folder,
                                                 num_iterations, buffer_size, logger=None):
    """Wrapper that catches failures and returns None for failed images.

    Accepts same arguments as crop_mammogram_one_image_short_path plus logger.
    """
    try:
        ...
        (logger or print)(f"CROP FAILED: {scan.get('short_file_path', 'unknown')} - {e}")
        return None

def crop_mammogram(input_data_folder, exam_list_path, cropped_exam_list_path, output_data_folder,
                   num_processes, num_iterations, buffer_size, error_log_path=None, logger=None, tolerant=True):
    """
    ...
    log(f"success={ok}/{total} images; failures={total-ok}; out_dir={output_data_folder}")
        ...
        try:
            ...
            with open(error_log_path, "w") as f:
                for sid, err in failures:
                    f.write(f"{sid}\t{err}\n")
            log(f"wrote failure log: {error_log_path} ({len(failures)} items)")
        except Exception as e:
            log(f"could not write failure log: {e}")
    ...
    for rec in successes:
        try:
            wl_pairs.append(rec[0]); rp_pairs.append(rec[1]); bp_pairs.append(rec[2]); ds_pairs.append(rec[3])
        except Exception:
            continue
...
def crop_mammogram_one_image(scan, input_file_path, output_file_path, num_iterations, buffer_size, logger=None):
    """
    ...
    try:
        ...
        log = logger if logger else print
        log(f"FAIL short_id={scan.get('short_file_path')} path={input_file_path} err={error}")
        return {"failed": True, "error": str(error)}
    else:
        # existing crop window
        top, bottom, left, right = cropping_info[0]

        # new: enforce paper size
...
def crop_mammogram_one_image_short_path(scan, input_data_folder, output_data_folder,
    ...
    if logger:
        logger(f"START short_id={short_id} in={full_input_file_path}")
    ...
    if logger and isinstance(cropping_info, tuple) and len(cropping_info) == 4:
